...
public class S3Profile {
    ...
    private final Secret sessionToken;
    ...
    public S3Profile(String name, String accessKey, String secretKey, String sessionToken, boolean useRole, int signedUrlExpirySeconds, String maxUploadRetries, String uploadRetryTime, String maxDownloadRetries, String downloadRetryTime, boolean keepStructure) {
        this.name = name;
        ...
        if (useRole) {
            ...
            this.sessionToken = Secret.fromString(sessionToken);
        }
    ...
    public final Secret getSessionToken() {
        return sessionToken;
    }
    ...
    public AmazonS3Client getClient(String region) {
        return ClientHelper.createClient(accessKey, Secret.toString(secretKey), Secret.toString(sessionToken), useRole, region, getProxy());
    }
    ...
    public List<FingerprintRecord> upload(Run<?, ?> run,
        ...
        try {
            for (int i = 0; i < fileNames.size(); i++) {
                ...
                if (gzipFiles) {
                    upload = new S3GzipCallable(accessKey, secretKey, sessionToken, useRole, dest, userMetadata,
                            storageClass, selregion, useServerSideEncryption, getProxy());
                } else {
                    upload = new S3UploadCallable(accessKey, secretKey, sessionToken, useRole, dest, userMetadata,
                            storageClass, selregion, useServerSideEncryption, getProxy());
                }
          ...
          for(final FingerprintRecord record : artifacts) {
              ...
              if (FileHelper.selected(includeFilter, excludeFilter, artifact.getName())) {
                  fingerprints.add(repeat(maxDownloadRetries, downloadRetryTime, dest, new Callable<FingerprintRecord>() {
                      @Override
                      public FingerprintRecord call() throws IOException, InterruptedException {
                          final String md5 = target.act(new S3DownloadCallable(accessKey, secretKey, sessionToken, useRole, dest, artifact.getRegion(), getProxy()));
