...
abstract class S3Callable<T> implements FileCallable<T> {
    ...
    S3Callable(String accessKey, Secret secretKey, Secret sessionToken, boolean useRole, String region, ProxyConfiguration proxy) {
    ...
    protected synchronized TransferManager getTransferManager() {
        ...
        if (transferManagers.get(uniqueKey) == null) {
            final AmazonS3 client = ClientHelper.createClient(accessKey, Secret.toString(secretKey), Secret.toString(sessionToken), useRole, region, proxy);
